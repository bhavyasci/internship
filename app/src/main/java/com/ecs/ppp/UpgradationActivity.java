...
public class UpgradationActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.upgradation);
		Log.d("Sync Process Started", "******");

		loadAllUsers();
		loadInputTypes();
		callProgressBar("Loading");
		
		// FlurryAgent.onStartSession(this, "2BXT5DDPKV95M8QBMV6R");
	...
	public JSONArray getRequestData() {
		...
		try {
			...
			if (callData == 6) {// save all result data to server
				Log.d("Update serverRecordList:", "" + updateServerRecordsDB);
				JSONObject request = new JSONObject();
				request.put("from", deviceID);
			...
			String responseString = WebServerCall.putDataToServer(requestJson);
	...
	private void progressBar()
	{
		try {
			...
			internet = new NetConnection(UpgradationActivity.this);
		 ...
		 startActivity(new Intent(UpgradationActivity.this, HomeActivity.class));
		 UpgradationActivity.this.finish();
	}
